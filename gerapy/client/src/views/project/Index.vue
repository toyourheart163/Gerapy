<template>
	<div class="panel" id="project-index">
		<el-dialog :visible.sync="createProjectDialog" size="tiny" width="auto" class="dialog">
				...
				<el-tab-pane :label="$lang.columns.clone" name="clone">
					<el-form>
						<el-form-item :label="$lang.columns.address" label-width="65px">
							<el-input
									v-model="gitAddress" class="inline" :placeholder="$lang.columns.address"
									size="small">
							</el-input>
						</el-form-item>
					</el-form>
			...
			<div slot="footer">
				...
				<el-button @click="onCreateProject()" v-if="activeDialogTab === 'create'"
					type="primary" size="small">{{ $lang.buttons.create }}
				</el-button>
				<el-button @click="onUploadedProject()" v-if="activeDialogTab === 'upload'"
					type="primary" size="small">{{ $lang.buttons.finish }}
				</el-button>
				<el-button @click="onCloneProject()" v-if="activeDialogTab === 'clone'"
					type="primary" size="small">{{ $lang.buttons.clone }}
				</el-button>
			</div>
		</el-dialog>
		...
		<div class="panel-body">
			<el-table
				...
				<el-table-column :label="$lang.columns.open">
					<template slot-scope="scope">
						<el-switch
							...
							@change="handlerProjectOpen(scope.row.id, scope.row.open)"
							:disabled="isOwner(scope.row.username)"
						></el-switch>
					</template>
				</el-table-column>
				<el-table-column align="center" :label="$lang.columns.configurable" width="120">
				...
				<el-table-column align="center" :label="$lang.columns.built" width="60">
					<template slot-scope="props" v-if="buildInfos[props.row.name]">
				...
				<el-table-column align="center" :label="$lang.columns.builtAt" width="150">
				...
				<el-table-column align="center" :label="$lang.columns.description" width="200">
					<template slot-scope="props">
						<span v-if="buildInfos[props.row.name]">
...
export default {
	...
	methods: {
		...
		getProjectData() {
			this.loading = true
			this.$http.get(this.$store.state.url.project.index).then(({
				data: projects
			}) => {
		...
		onRefresh() {
			//this.$message.info(this.$lang.messages.loading)
			this.getProjectData()
		},
		...
		onCloneProject() {
			this.$message.success(this.$lang.messages.cloning)
			this.$http.post(this.$store.state.url.project.clone, {
				address: this.gitAddress
			}).then(() => {
				this.$message.success(this.$lang.messages.successClone)
				this.loading = false
				this.createProjectDialog = false
				this.onRefresh()
			}).catch(() => {
				this.loading = false
				this.$message.error(this.$lang.messages.errorClone)
			})
		},
		onCreateProject() {
      ...
      return this.$store.state.auth.user != username
		...
		handlerProjectOpen(id, open) {
			this.$http.patch(this.formatString(this.$store.state.url.project.update, {
					id: id
				}), {open: open}
			).then(() => {
